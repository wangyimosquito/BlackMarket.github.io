...
#多线索集合挖掘函数
	...
	def gen_node_link(self):
		node_path = "./reduced_subGraph2/node"
		link_path = "./reduced_subGraph2/link"
		for i in [1,2,3,4,5]:
			with open(node_path + str(i) +".json", "r") as json_file:
				node = json.load(json_file)
			with open(link_path + str(i) +".json", "r") as json_file:
				link = json.load(json_file)
			print("group"+str(i)+":", len(node), len(link))
	...
	def member_info(self):
		...
		for i in [1,2,3,4,5]:
			...
			member_list  = []
			for each_node in node:
				if(self.node_json[each_node]['type'] == 'Whois_Name'):
					member_list.append(self.node_json[each_node]['name'])
			print("group"+str(i)+" member: ", member_list)
	
	def gen_dirty_node(self):
		node_path = "./reduced_subGraph2/node"
		link_path = "./reduced_subGraph2/link"
		for i in [1,2,3,4,5]:
			with open(node_path + str(i) +".json", "r") as json_file:
			...
			for each_node in node:
				if(self.node_json[each_node]['type'] == 'Domain'):
					if(len(self.node_json[each_node]['industry'])!= 0):
						dirty_node_list.append(each_node)
			save_path = './dirty_node/dirty_node'+str(i)+'.json'
			dict_json = json.dumps(dirty_node_list)
			with open(save_path,'w+') as file:
				file.write(dict_json)
	
	def count_dirty(self):
		node_path = "./dirty_node/dirty_node"
		for i in [1,2,3,4,5]:
			with open(node_path + str(i) +".json", "r") as json_file:
				node = json.load(json_file)
			print("group"+str(i)+":", len(node))

	def gen_group_member_info(self):
		node_path = "./reduced_subGraph2/node"
		for i in [1,2,3,4,5]:
			with open(node_path + str(i) +".json", "r") as json_file:
				node = json.load(json_file)
			...
			save_path = './group_member_list/group_info'+str(i)+'.json'
			dict_json = json.dumps(member_info_list)
			print("group"+str(i), len(member_info_list))
			with open(save_path,'w+') as file:
				file.write(dict_json)
...
if(__name__ == '__main__'):
	...
	#t.statistic_gen("./reduced_subGraph2/node5.json")
	t.gen_group_member_info()
